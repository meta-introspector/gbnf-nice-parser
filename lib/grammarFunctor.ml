...
module Nonterminal = struct
  ...
  let () =
    StringMap.iter (fun nonterminal { attributes = attrs; _ } ->
...
module Production = struct
  ...
  let n : int =
    let n = StringMap.fold (fun _ { branches = branches ; _} n ->
  ...
  let (_ : int) = StringMap.fold (fun nonterminal { branches; _ } k ->
...
(* We need to determine which symbols generate a nonempty word, that is, a
   word of length at least 1. This could be done directly by a fixed point
   computation over a Boolean lattice, but it is easier to just rely on the
   FIRST sets, which we compute anyway. A symbol generates a nonempty word if
   and only if its FIRST set is nonempty. *)

let generates_nonempty_word symbol : bool =
  not (TerminalSet.is_empty (FIRST.symbol symbol))
