...
(*   (\* (\\* Positions.with_pos pos (EAction (XATraditional raw_action2, None)) *\\) *\) *)
(*    EAction (action, None)  *)
...
: parameterized_branch =
  ...
  (*     (\* (\\* Done. *\\) *\) *)
  (*     let e = (EAction (XATraditional (Action.from_il_expr (EVar "None")), None)) in *)
  (*     let e2 = Positions.with_pos pos e in *)
  (*     production_aux context  e2  level  *)
